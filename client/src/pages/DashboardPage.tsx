...
export default function DashboardPage() {
    const [farmId, setFarmId] = useState(() => localStorage.getItem("farmId") ?? DEFAULT_FARM_ID);
    const [windmills, setWindmills] = useState<Windmill[]>([]);
    const [selected, setSelected] = useState<Windmill | null>(null);

    const canLoad = useMemo(() => farmId.trim().length > 0, [farmId]);
    ...
    const loadWindmills = async () => {
        const list = await api.windmills.getWindmills(farmId);
        setWindmills(list ?? []);
            ...
            <section className="card">
                <div className="row">
                    <label style={{ display: "grid", gap: 6, width: 320 }}>
                        ...
                        <input value={farmId} onChange={(e) => setFarmId(e.target.value)} placeholder={DEFAULT_FARM_ID} />
                    </label>

                    <button className="btn btn-primary" disabled={!canLoad} onClick={() => loadWindmills().catch(() => {})}>
                        Load windmills
                    </button>
                ...
                <main className="panelGrid">
                    ...
                    <section className="card">
                        ...
                        {!selected ? (
                            ...
                            <div style={{ display: "grid", gap: 10 }}>
                                {filteredAlerts.slice().reverse().map((a) => (
                                    <div className="alertItem" key={a.id}>
                                        <div className="alertTop">
                                            <strong>{(a.severity ?? "info").toUpperCase()}</strong>
                                            <span className="muted">{a.timestamp ? new Date(a.timestamp).toLocaleString() : ""}</span>
                                        </div>
                    ...
                    <section className="card">
                        ...
                        {!selected ? (
                            ...
                            <ControlsPanel farmId={selected.farmId!} turbineId={selected.id!} />
                        )}
                    </section>
                </main>
            </div>
